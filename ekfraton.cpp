...
int main(int argc, char ** argv) {
	...
	Mat_<float> x_est(4, 1); /* (x, y, Vx, Vy) */
x_est= (Mat_<float>(4,1) << 0, 0, 0, 0);
...
//-------------------------------------------
	...
	for (;;)
	{
		...
		f = (Mat_<float>(4,1) << x(0) , x(1) , 0, 0);
		H = (Mat_<float>(2,4) <<1, 0, 0, 0, 0, 1, 0, 0); //jacobiana de la funcion h
		transpose(A, At);
		transpose(W, Wt);
		transpose(V, Vt);
		transpose(H, Ht);
		...
		for (;;) {
			...
			float deltat = (float)getTickCount();
			transpose(A, At);
			//--------------Predicción------------------
			...
			///kalmanv.push_back(x);
			kalmanv.push_back(Point2f(x_est.at<float>(0), x_est.at<float>(1)));

			// plot points

			img = Scalar::all(0);
			drawCross(img, Point2f(x_est.at<float>(0), x_est.at<float>(1)), Scalar(255, 255, 255), 5);
			...
			deltat = ((float)getTickCount() - deltat)/getTickFrequency();
	
			A = (Mat_<float>(4,4) <<1, 0, deltat, 0, 0, 1, 0, deltat, 0, 0, 1, 0, 0, 0, 0, 1);
			f = (Mat_<float>(4,1) << x_est(0) + (deltat*x_est(2)), x_est(1) + deltat*x_est(3), x_est(2), x_est(3));
cout<<f<<endl;

			h = (Mat_<float>(2,1) <<x_est(0), x_est(1));
